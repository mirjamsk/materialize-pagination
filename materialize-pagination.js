...
(function($, window, document, undefined) {
	...
	MaterializePagination.prototype = {
		...
		requestPage: function(requestedPage, initing, isNeedUpdateUrlParam) {
			isNeedUpdateUrlParam = isNeedUpdateUrlParam === undefined ? true : isNeedUpdateUrlParam;
			...
			if (!initing)
				this.config.onClickCallback.call(this, this.currentPage);
			this.renderActivePage();

			if (this.config.useUrlParameter && isNeedUpdateUrlParam)
		...
		renderEllipsis: function() {
			...
			if (this.$nextEllipsis.isHidden &&
				this.currentPage <= this.config.lastPage - this.config.maxVisiblePages)
		...
		bindClickEvent: function() {
			...
			this.$container.on('click', 'li', function() {
				if ($(this).data('page') !== undefined) {
					...
					if (self.options.siblings && self.options.siblings.each) {
						self.options.siblings.each(function (i, el) {
							if (el.materializePagination
								&& el.materializePagination instanceof MaterializePagination
								&& el.materializePagination.currentPage !== requestedPage) {
								el.materializePagination.requestPage(requestedPage, true, false);
							}
						});
					}
				}
	...
	$.fn.materializePagination = function(options) {
		var self = this;
		var typeOfOptionsSiblings = typeof options.siblings;

		return this.each(function(i, el) {
			var _options = {};
			if (typeOfOptionsSiblings === 'boolean' && options.siblings) {
				_options.siblings = self.slice();
				_options.siblings.splice(i, 1);
			}

			el.materializePagination = new MaterializePagination(this, $.extend({}, options, _options));
			el.materializePagination.init();
		});
	};
